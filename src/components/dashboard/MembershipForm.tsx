...
export default function MembershipForm({ membership, onClose, onSuccess }: MembershipFormProps) {
  ...
  useEffect(() => {
    ...
    if (membership) {
      ...
      if (membership.member_type === 'learner') {
        fetchMembershipCourses(membership.id)
      } else {
        fetchMembershipPromotions(membership.id)
      }
    }
  }, [membership])
  ...
  const fetchMembershipCourses = async (membershipId: string) => {
    try {
      const { data, error } = await supabase
  ...
  const fetchMembershipPromotions = async (membershipId: string) => {
    try {
      const { data, error } = await supabase
        .from('membership_promotion_rights')
        .select('promotable_membership_id, commission_rate')
        .eq('membership_package_id', membershipId)
      ...
      console.error('Error fetching membership promotions:', error)
    }
  }
  ...
  const handlePromotionToggle = (membershipId: string) => {
    setFormData(prev => {
      const isSelected = prev.selectedPromotions.includes(membershipId)
      const newPromotions = isSelected
        ? prev.selectedPromotions.filter(id => id !== membershipId)
        : [...prev.selectedPromotions, membershipId]
      
      const newRates = { ...prev.commissionRates }
      if (!isSelected) {
        newRates[membershipId] = 100 // Default commission rate
      } else {
        delete newRates[membershipId]
      }
      
      return {
        ...prev,
        selectedPromotions: newPromotions,
        commissionRates: newRates
      }
    })
  }

  const handleCommissionRateChange = (membershipId: string, rate: number) => {
    setFormData(prev => ({
      ...prev,
      commissionRates: {
  ...
  const validateForm = () => {
    ...
    if (formData.member_type === 'learner' && formData.selectedCourses.length === 0) {
      toast.error('Please select at least one course for learner membership')
      return false
    }
    if (formData.member_type === 'affiliate' && formData.selectedPromotions.length === 0) {
      toast.error('Please select at least one membership to promote for affiliate membership')
      return false
    }
    return true
  }
      ...
      if (formData.member_type === 'affiliate') {
        ...
        if (formData.selectedPromotions.length > 0) {
          const promotionRights = formData.selectedPromotions.map(promotableId => ({
            ...
            commission_rate: formData.commissionRates[promotableId] || 100
          }))
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Add features and benefits that come with this membership
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
            ...
            {formData.member_type === 'learner' ? (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Select which courses members will have access to
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {courses.length === 0 ? (
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Select which learner memberships this affiliate can promote and set commission rates
                  </CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    <div className="space-y-3">
                      {learnerMemberships.map((learnerMembership) => (
                        ...
                        >
                          ...
                          {formData.selectedPromotions.includes(learnerMembership.id) && (
                            <div className="flex items-center space-x-2">
                              <Label htmlFor={`rate-${learnerMembership.id}`} className="text-xs">
                                Commission %:
                              </Label>
                              <Input
                                id={`rate-${learnerMembership.id}`}
                                type="number"
                                min="0"
                                max="100"
                                step="0.01"
                                value={formData.commissionRates[learnerMembership.id] || 100}
